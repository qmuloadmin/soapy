...
class EnvElement(Element):

	""" Class containing attributes and properties of an element in a SOAP Envelope created from WSDL """
	...
	def attributes(self):
		try:
			...
			#TODO collapse non-Element child classes that define attributes here
...
class ComplexType(Element):
	...
	def parentAttributes(self):
		try:
			...
			self._log("Initializing list of attributes for parent element",5)
			attrs = list()
			attributes = self.bsElement('attribute',recursive=False)
			for attribute in attributes:
				attrs.append(Attribute(attribute,self.wsdl))
...
class SequenceType(Element):
	...
	@property
	def name(self):
		raise AttributeError("Sequence type has no attribute 'name'")

class Element():
	
	""" Base class for handling instantiation and name attribute for any WSDL element """

	def __init__(self,bsElement,wsdl):
	...
	def fromName(cls,name,wsdl):
		...
		ports = wsdl.wsdl(tag,recursive=False)
		wsdl._log("Searching for {1} element with name matching {0}"
					.format(name,cls.__name__),5)
		for port in ports:
			if port.get('name') == name:
				return cls(port,wsdl)
...
class Wsdl(Log):
	...
	def __init__(self,wsdl_location,tracelevel=1,**kArgs):
		...
		the XML each time, then provide keyword args of cache=FH where FH is a file handler """
	...
	def __str__(self):
		return str(self.wsdl)

	def typeFactory(self,element):
		
		""" Factory that creates the appropriate EnvClass based on bsElement tag """

		if element.name == "element":
			return EnvElement(element,self)
		elif element.name == "complexType":
			return ComplexType(element,self)
		elif element.name == "sequence":
			return SequenceType(selement,self)
		else:
			raise TypeError("XML Element Type Tag {0} not yet implemented".format(element.name))
	...
	def ports(self):
		try:
			...
			for port in self.bsElement('port',recursive=False):
				ports.append(Port(port,self.wsdl))
...
class Port(Element):
	...
	def binding(self):
		try:
			...
			self.__binding = Binding.fromName(name,self.wsdl)
			return self.__binding
...
class Binding(Element):
	...
	def type(self):
		try:
			...
			self.__type = PortType.fromName(name,self.wsdl)
			return self.__type

	def getSoapAction(self,opName):
	...
	def operations(self):
		try:
			...
			for operation in self.bsElement('operation',recursive=False):
				operations.append(Operation(operation,self.wsdl))
	...
	def parts(self):
		try:
			...
			for part in self.bsElement('part',recursive=False):
				parts.append(Part(part,self.wsdl))
...
class Part(Element):
	...
	@property
	def type(self):
		return self.bsElement['element'].split(':')[1]
...
class Operation(Element):
	...
	def input(self):
		try:
			...
			self.__input = Message.fromName(name,self.wsdl)
			return self.__input
	...
	def output(self):
		try: 
			...
			self.__output = Message.fromName(name,self.wsdl)
			return self.__output
	...
	def fault(self):
		try:
			...
			if len(searchResults) == 1:
				...
				self.__fault = Message.fromName(name,self.wsdl)
				return self.__fault
			else:
